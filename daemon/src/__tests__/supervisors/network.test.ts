...
describe('NetworkSupervisor', () => {
  ...
  function createTestSupervisor(port: number, backoffOptions?: object): NetworkSupervisor {
    const sup = createNetworkSupervisor({
      config: mockConfig,
      wsUrl: `ws://localhost:${port}`,
      backoff: { maxRetries: 3, baseDelayMs: 10, jitterFactor: 0, ...backoffOptions },
    });
  ...
  describe('event handling', () => {
    ...
    it('emits completed event', async () => {
      ...
      const completedHandler = mock((_itemId: string, _transcript: string) => {});
      ...
      expect(completedHandler).toHaveBeenCalledWith('test_item_1', 'Hello, world!');
    });
  ...
  describe('sendAudio', () => {
    it('sends audio when connected', async () => {
      ...
      expect((receivedMessage as { audio: string }).audio).toBe('dGVzdCBhdWRpbw==');
    });
