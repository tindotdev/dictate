...
export interface NetworkSupervisorEvents {
	// State events
	state_change: (
		from: NetworkSupervisorState,
		to: NetworkSupervisorState,
	) => void;
	connected: () => void;
	disconnected: () => void;
...
export declare interface NetworkSupervisor {
	on<K extends keyof NetworkSupervisorEvents>(
		event: K,
		listener: NetworkSupervisorEvents[K],
	): this;
...
export class NetworkSupervisor extends EventEmitter {
	...
	private handleClose(code: number, reason: string): void {
		...
		if (delayMs === null) {
			...
			this.emit(
				"failed",
				new Error(
					`WebSocket failed after ${this.backoffState.config.maxRetries} retries: ${code} ${reason}`,
				),
			);
			return;
		}
	...
	private sendSessionUpdate(): void {
		// Use the flat session format for gpt-4o-realtime-preview
...
export function createNetworkSupervisor(
	options: NetworkSupervisorOptions,
): NetworkSupervisor {
